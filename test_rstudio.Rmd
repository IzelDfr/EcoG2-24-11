...
Examiner l'analyse des coordonnées principales (PCoA) avec la dissemblance de Bray-Curtis sur la distance Unifrac pondérée.
```{r}
...
qplot(rel_abund[, 12], geom = "histogram",binwidth=0.05) + xlab("Relative abundance")
````
...
#regarder les résultats de PCoA avec Unifrac pondéré
out.wuf.log <- ordinate(pslog, method = "PCoA", distance ="wunifrac")
evals <- out.wuf.log$values$Eigenvalues
plot_ordination(pslog, out.wuf.log, color = "age_binned",shape = "family_relationship") + coord_fixed(sqrt(evals[2] / evals[1])) + labs(col = "Binned Age", shape = "Litter")
````
...
```{r}
#spéciifier quelle caractéristiques est à considérer
...
```{r}
library(vegan)
pls_biplot <- list("loadings" = loadings(plsFit$finalModel), "scores" = scores(plsFit$finalModel))
...
Identifier quel organisme a le plus d'influence dans la prédiction du RandomForest
```{r}
